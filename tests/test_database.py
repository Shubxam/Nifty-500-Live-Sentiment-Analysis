...
def test_insert_articles_no_sentiment(db_manager):
    ...
    with db_manager.get_connection() as conn:
        df = conn.execute("SELECT * FROM article_data WHERE compound_sentiment IS NULL").fetchdf()
        assert len(df) == 2
...
def test_insert_articles_with_sentiment(db_manager):
    ...
    with db_manager.get_connection() as conn:
        ...
        assert not df["compound_sentiment"].isnull().any()  # Check sentiment columns are NOT NULL
...
def test_get_articles(db_manager):
    ...
    latest_with_sentiment = db_manager.get_articles(n=1, latest=True, has_sentiment=True)
    ...
    oldest_no_sentiment = db_manager.get_articles(n=1, latest=False, has_sentiment=False)
    ...
    assert all_latest["date_posted"].is_monotonic_decreasing  # Dates should be descending
